...
for i in range(3):
    ...
    for num, ax in enumerate(axs.ravel()):
        ...
        ax.axis('off')
    # plt.show()
    fig.savefig('figures/mnist_{}.png'.format(['original', 'white_noise', 'blur'][i]))
...
rej_perc_df.to_csv('out/{}/rp_additive.csv'.format(ts))
...
df = pd.DataFrame(data)
rej_perc_df = df.groupby(["sigma", "test_name"])["value"].mean().reset_index()
rej_perc_df.to_csv('out/{}/rp_blurred.csv'.format(ts))


# -------------------------------- Plot percentage of rejections vs  Noise  --------------------------------
fig, [axl, axr] = plt.subplots(figsize=(12, 6), ncols=2)
df_additive = pd.read_csv('out/mnist/{}/rp_additive.csv'.format(ts))
df_blurred = pd.read_csv('out/mnist/{}/rp_blurred.csv'.format(ts))
