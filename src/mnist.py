...
def run_perturbed_minst_test(X, Y, num_permutations, test_names):
    ...
    for test in test_names:
        if test == 'MMD':
            ...
            out.append(( test, 1 if GKE_KL_two_sample_test(X,Y, kappa_K = kappa_K)(num_permutations) < 0.05 else 0 ) )
        elif test == '*HS*':
            out.append(( test, 1 if GKE_HS_two_sample_test(X,Y, kappa_K = kappa_K)(num_permutations) < 0.05 else 0 ) )
        elif test == 'KNN':
