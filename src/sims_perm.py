...
test_names = ['*KL*', '*HS*', 'MMD', 'KNN', 'FR', 'HT']
...
for _model_, model_params in lsmodels:
    ...
    for n, d in n_d_values:
        decisions_list = run_parallel(n, d, _model_, model_params, test_names, num_permutations, N_iters, NUM_CORES)
        results.append(decisions_list)
    out = []
    for name in test_names:
        for _, (n,d) in zip(results, n_d_values):
            out.append( [name, n,d, N_iters, num_permutations,  pd.DataFrame(_).mean(0)[name] ])
    ...
    data_out = pd.DataFrame(out, 
                columns= ['test', 'sample size', 'dimension', 'N_iters', 'num_permutations', 'percent of rejections']
                )
