...
#         if test_name == '*KL*':
#             values.append(GKE_KL_two_sample_test(X,Y, kappa_K = kappa_K).obs_value)
#         if test_name == '*HS*':
#             values.append(GKE_HS_two_sample_test(X,Y, kappa_K = kappa_K).obs_value)
#         if test_name == 'MMD':
...
# all_CovKerEmb_tests = ['*HS*', '*KL*', 'KNN', 'FR', 'HT', 'MMD', 'MPZ']
...
#                 bandwidth = 2 * median_dist * band_factor
...
#             SX, SX_ED, SY, SY_ED = get_Smats_X_Y(kxx, kxy, kyy, kappa_K)
#             # values.append(CovKerEmb_test_stat(test_name, SX, SY, SX_ED, SY_ED))
#         if test_name == 'MMD':
#             values.append(MMD_two_sample_test(X,Y).obs_value)
#         if test_name == 'KNN':
#             values.append(KNN_two_sample_test(X,Y).obs_value)
