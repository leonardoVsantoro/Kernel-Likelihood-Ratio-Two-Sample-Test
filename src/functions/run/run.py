...
def run_fast(X,Y, num_permutations, kernel_name, ridge, band_factor, light = False, symmetrise = True, project = True, return_all = False):
    n = len(X); m = len(Y)
    ...
    for BF in band_factor:
        ...
        for r in ridge:
            ...
            obs_value['KLR-0', r, BF] = HS_term
            obs_value['CM', r, BF] = CM_term
            obs_value['SpecReg-MMD', r, BF] = np.linalg.norm(inv_sqrtm_ED(C_ED)@(mY-mX))
            obs_value['logdet2', r, BF] = logdet2_term
        ...
        for _ in range(num_permutations):
            ...
            if symmetrise:
                _SX = .5*(_SX + _SY); _mX = .5*(_mX + _mY)
            _eigvals_SX, _eigvecs_SX = np.linalg.eigh(_SX)
            if project:
                _projection_matrix = _eigvecs_SX[:, :] @ _eigvecs_SX[:, :].T
                _SY = _projection_matrix @ _SY @ _projection_matrix.T
            _C = get_Cmat(_kernel_matrix)
            _eigvals_C, _eigvecs_C = np.linalg.eigh(_C)
            permuted_values['AggMMD', BF].append(_kxx.sum()/(n*(n-1)) +  _kyy.sum()/(m*(m-1)) - 2* _kxy.sum()/(n*m))
            for r in  ridge:
                ...
                permuted_values['KLR-0', r, BF].append(_HSterm)
                permuted_values['CM', r, BF].append( _CM_term)
                permuted_values['SpecReg-MMD', r, BF].append(np.linalg.norm(inv_sqrtm_ED(_C_ED)@(_mY-_mX)))
                permuted_values['logdet2', r, BF].append(_logdet2_term)
        
        adj_obs_value = {key: obs_value[key] if np.std(vals) > 1e-12 else 0 for key, vals in permuted_values.items()}
    ...
    p_values['KLR'] = [np.mean(np.array(permuted_values['logdet2', r, BF]) + np.array(permuted_values['CM', r, BF]) > adj_obs_value['logdet2', r, BF] + adj_obs_value['CM', r, BF])
                           for r in ridge for BF in band_factor]
