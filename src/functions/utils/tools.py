...
def get_mvecs_X_Y(kxx, kxy, kyy):
    ...
    n = kxx.shape[0]
    m = kyy.shape[0]
    mX = np.concatenate([kxx, kyx]).sum(1)
    mY = np.concatenate([kxy, kyy]).sum(1)
    return mX, mY


def get_Smats_X_Y(kxx, kxy, kyy, regularisation_param = None):
    kyx = kxy.T
    n = kxx.shape[0]
    m = kyy.shape[0]
    if regularisation_param is None:
        regularisation_param = n**(-1/5)

    KmatX = np.concatenate([kxx, kyx])
    KmatY = np.concatenate([kxy, kyy])
    SX = KmatX @ (KmatX.T) / n + regularisation_param*np.eye(m+n)
    SY = KmatY @ (KmatY.T) / n + regularisation_param*np.eye(m+n)
    SX_ED = EIG_DEC(SX)
    SY_ED = EIG_DEC(SY)
    return SX, SX_ED, SY, SY_ED

# ------------------------ # ------------------------ # ------------------------ # ------------------------ 


def carleman_fredholm_determinant(A):
